...
def make_int_prop(name, prop_tab=None, prop_server=None):
    return make_prop(
        name, prop_tab=prop_tab, prop_server=prop_server, prop_args=["MustBeInt:=", True]
    )


def make_float_prop(name, prop_tab=None, prop_server=None):
    return make_prop(
        name, prop_tab=prop_tab, prop_server=prop_server, prop_args=["MustBeInt:=", False]
    )
        ...
        self.prop_holder.ChangeProperty(
            [
                ...
                [
                    ...
                    ["NAME:ChangedProps", ["NAME:" + name, "Value:=", value] + prop_args],
                ],
            ]
        )
                ...
                eval(
                    str(
                        sympy_parser.parse_expr(
                            str(sympy_parser.parse_expr(str(name), self.variables)), self.variables
                        )
                    )
                )
            )
...
class HfssModeler(COMWrapper):
    ...
    def polyline(self, points, closed=True, **kwargs):
        ...
        for ii, point in enumerate(points):
            pointsStr.append(
                ["NAME:PLPoint", "X:=", str(point[0]), "Y:=", str(point[1]), "Z:=", str(point[2])]
            )
            indexsStr.append(
                ["NAME:PLSegment", "SegmentType:=", "Line", "StartIndex:=", ii, "NoOfPoints:=", 2]
            )
        if closed:
            pointsStr.append(
                [
    ...
    def duplicate_along_line(self, entity, vec, n=2):
        self._modeler.DuplicateAlongLine(
            ["NAME:Selections", "Selections:=", entity.name, "NewPartsModelFlag:=", "Model"],
            [
    ...
    def assign_perfect_E_faces(self, entity):
        ...
        self._boundaries.AssignPerfectE(
            ["NAME:" + str(name) + '_PerfE', "Faces:=", faces_perfE, "InfGroundPlane:=", False]
        )
    ...
    def assign_waveport(self, entity, name, Nmodes, DoRenorm, RenormValue, DoDeembed, DeembedDist):
        ...
        for n in range(Nmodes):
            inlinearray = ["NAME:IntLine", "Start:=", start, "End:=", stop, "CharImp:=", "Zpi"]
            modesarray.append(
                [
                    "NAME:Mode" + str(n + 1),
                    "ModeNum:=",
    ...
    def assign_mesh_length(self, entities, length):
        ...
        params += ["RestrictElem:=", False, "RestrictLength:=", True, "MaxLength:=", str(length)]
    ...
    def rotate(self, entities, angle, center=None, axis='Z'):
        ...
        self._modeler.Rotate(
            self._selections_array(*names),
            ["NAME:RotateParameters", "RotateAxis:=", axis, "RotateAngle:=", "%ddeg" % (angle)],
        )
    ...
    def rotate_x(self, entities, angle):
        ...
        self._modeler.Rotate(
            self._selections_array(*names),
            ["NAME:RotateParameters", "RotateAxis:=", "X", "RotateAngle:=", "%ddeg" % (angle)],
        )
    ...
    def rotate_y(self, entities, angle):
        ...
        self._modeler.Rotate(
            self._selections_array(*names),
            ["NAME:RotateParameters", "RotateAxis:=", "Y", "RotateAngle:=", "%ddeg" % (angle)],
        )
