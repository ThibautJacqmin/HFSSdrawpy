        ...
        if ebeam:
            if 1:
                chip1.set_current_coor(
                    [PM.x_I, PM.y_I - 0.5 * PM.cutout_depth + 3 * PM.buffer + PM.cap_depth], [0, 1]
                )
                chip1.draw_cos2phi(
                    'junction_parity',
                    ['5um', '5um'],
            ...
            if 1:
                chip1.set_current_coor(
                    [PM.x_I, PM.y_I - 0.5 * PM.cutout_depth + PM.buffer], Vector([0, 1]).orth()
                )
                chip1.draw_dose_test_junction(
                    'ind_parity',
                    ['5um', '5um'],
...
if memory and parity:
    chip1.draw_cable(
        'mem', 'parity_portOut1', 'trm_portOut1', is_bond=is_bond, fillet=PM.fillet, is_mesh=True
    )
...
if flux_lines:
    ...
    chip1.set_current_coor(
        [PM.x_I + PM.offset, PM.y_I + 0.5 * PM.cutout_depth - 1.75 * PM.approach], [0, 1]
    )
    chip1.draw_fluxline(
        'flux_top',
        PM.track,
    ...
    chip1.set_current_coor(
        [PM.x_I + 0.5 * PM.cutout_width - PM.approach, PM.y_I - 0.55 * PM.offset], [1, 0]
    )
    chip1.draw_fluxline(
        'flux_bottom',
        PM.track,
